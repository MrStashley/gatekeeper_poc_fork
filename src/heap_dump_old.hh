 ...
 *     heap_walk_known_structs();     // search for known types
 */
...
/* ====================================================================
 * Connection-specific search with std::string buffer tracking
 * ====================================================================
 *
 * Two-pass approach:
 *   Pass 1: Find all Connection objects by vptr. For each one, read
 *           request_buffer.data() and response_buffer.data(). If the
 *           pointer falls outside the Connection's own chunk (i.e. not
 *           SSO), record it as a tracked buffer.
 *   Pass 2: Walk the heap for display. Label Connection chunks,
 *           label string-buffer chunks, histogram everything else.
 * ==================================================================== */
...
typedef struct hw_tracked_buf {
    unsigned    conn_id;
    const char *label;       /* "request_buffer" or "response_buffer" */
    uintptr_t   data_ptr;   /* string.data() — should match some chunk's user_ptr */
...
/*
 * Check whether a std::string's data pointer is SSO (points inside
 * the containing object's chunk) or heap-allocated (points elsewhere).
 */
...
static int heap_find_connections(void)
{
    uintptr_t heap_start, heap_end;
    if (hw_find_heap_region(&heap_start, &heap_end) != 0)
        return -1;
    ...
    /* Grab the vptr from a default-constructed Connection */
    ...
    hw_tracked_buf_t tracked[HW_MAX_CONNS * 2]; /* at most 2 strings per conn */
    unsigned n_conns   = 0;
    ...
    while (cursor + HW_MIN_CHUNK_SIZE <= heap_end) {
        ...
        if (!is_top) {
                ...
                n_conns < HW_MAX_CONNS)
            {
                ...
                /* Cast and read the string fields */
    ...
    cursor = first;
    unsigned idx       = 0;
    unsigned conn_idx  = 0;  /* next Connection to match */
    ...
    while (cursor + HW_MIN_CHUNK_SIZE <= heap_end) {
        ...
        /* Check: is this a Connection chunk? */
        int is_conn = 0;
        ...
        /* Check: is this a tracked string buffer? */
        int is_strbuf = 0;
        ...
        if (is_conn) {
            ...
            /* Print summary of each string field for this connection */
            const http::Connection *conn =
                (const http::Connection *)user_ptr;
...
static int heap_walk_known_structs(void)
{
    return heap_find_connections();
}
